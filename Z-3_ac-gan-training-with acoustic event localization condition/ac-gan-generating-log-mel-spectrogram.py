...
def GetTrainData(GenerateType):
    ...
    Localization = np.zeros((int(np.sum(cnt))))
    print(cnt)
    
    count = 0
    ### Dat, Lab, Localization denote the acoustisc features, class condition c and event localization based condition d
    for ind in range(Classnum):
        if (ind != GenerateType):
            continue
        for i in range(label.shape[0]):         
            ...
            if((label[i,ind] == 1) & (a == 1)):
                Dat[count,:] = feat[i,:]
                Lab[count] = ind
                if((local[i,ind]>=0.3)&(local[i,ind]<0.4)):
                    Localization[count] = 1
                if((local[i,ind]>=0.4)&(local[i,ind]<0.5)):
                    Localization[count] = 2
                if((local[i,ind]>=0.5)&(local[i,ind]<0.6)):
                    Localization[count] = 3
                if((local[i,ind]>=0.6)&(local[i,ind]<0.7)):
                    Localization[count] = 4
                if((local[i,ind]>=0.7)&(local[i,ind]<0.8)):
                    Localization[count] = 5
                if((local[i,ind]>=0.8)&(local[i,ind]<0.9)):
                    Localization[count] = 6
                if((local[i,ind]>=0.9)&(local[i,ind]<1)):
                    Localization[count] = 7                                                                                                                       
                count = count + 1
    s = 0
    for ind in range(Classnum):
        if (ind != GenerateType):
    ...
    for ind in range(Classnum):
        ...
        for i in range(int(cnt[ind]/np.float(seq))):
            ...
            y_Dat[s] = ind  #### the format for y_Dat is one integer
            for tmp in range(40):
                y_Dat_d[s,tmp] = Localization[int(pos+i*seq)+tmp]      ### the format for y_Dat_d is:   [1 5 6 7 4 2 6 4 ... 3], the number in y_Dat_d denote the condition d in the paper in total consecutive frames.   
            s = s + 1
    X_Dat = np.expand_dims(X_Dat, axis=1)
    Conditions = np.zeros((s,40+1))
    Conditions[:,0] = y_Dat
    Conditions[:,1:41] = y_Dat_d
    return X_Dat,Conditions
...
def build_generator(latent_size):
    ...
    image_class = Input(shape=(41,), dtype='int32')  ##### first dimension for the class label, 1:41 denote the event localization based label of consecutive 40 frames
    ...
    for GenerateType  in [1,2,3,5,6]:   ### 1,2,3,5,6 denote the object snapping, cupboard, cutlery, drawer and glass jingling respectively.
        ...
        for epoch in range(nb_epochs):
            ...
            for index in range(nb_batches):
                ...
                sampled_labels = np.zeros((batch_size,41)) 
                for b in range(batch_size):                    
                    sampled_labels[b,0] = GenerateType  
                    for tmp in range([1:41]):
                        sampled_labels[b,tmp] = random.randint(0,7)  
            ...
            if(epoch % 100 == 0):
                ...
                for ind in range(11):                    
                    ...
                    sampled_labels = np.zeros((10000,41)) 
                    for b in range(10000):                    
                        sampled_labels[b,0] = GenerateType  
                        for tmp in range([1:41]):
                            sampled_labels[b,tmp] = random.randint(0,7)   ###1 to 41 denote the localization based condition of consecutive frames
                    ...
                    label = np.zeros((10000, seq, 11),dtype=int)   ### 11 here denote the number of event classes of interest
                    label[:,:,ind] = 1
